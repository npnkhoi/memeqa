...
def get_target_distractors(targets:List[str],
    ...
    alternate_targets = [t for t in SIMILAR_TARGETS.keys() if t not in all_similar_targets
                         and t not in old_distractors]
    random.shuffle(alternate_targets)
    
    return alternate_targets[:3]

class TargetSentimentId(AFBase):
    QUESTION_TEXT = 'Select all targets of this meme. Targets are entities that the meme is discussing.'
    ...
    def _get_init_questions(self) -> Tuple[List[Question] | Dict]:
        ...
        for img in self.img_set:
            ...
            targets:List[str] = []
            for i in intents:
                for _target in i["targets"]:
                    name = _target["name"]
                    if name not in targets: # contrasting intents
                        targets.append(name)
                        
            
            # get the list of actions that people used, these will not be added to the list of distractors
            targets = [_target.strip() for _target in targets]
            correct_options = targets.copy()
                
            potential_distractors = get_target_distractors(correct_options)
...
class TargetIdGenerator(NoModelGenerator):
    @override
    ...
    def _add_distractors(self, q: Question, new_distractors: List[str]) -> Question:
        ...
        if self.question_type == GENERAL_TYPE.SINGLE.value:
            ...
            q.options += new_distractors
            q.options = q.options[:4] # fixes unexpected crashing when more than 3 distractors are passed
            random.shuffle(q.options) # REMEMBER TO ALWAYS SHUFFLE THE OPTIONS
            q.answer_key = [
    ...
    def __init__(self, question_type = GENERAL_TYPE.SINGLE.value):
        self.GENERAL_TYPE = question_type
        super().__init__()
        self._get_distractors = get_target_distractors
...
if __name__ == "__main__":
    ...
    generator = TargetIdGenerator(question_type = GENERAL_TYPE.MULTIPLE)
