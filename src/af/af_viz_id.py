...
from src.af.data import IMG_FOLDER, MEME_INTENTS, OLD_QUESTIONS, SEMEVAL_IMAGES
...
class AFVisualId(AFBase):
    """
    Class for generating Visual Identification Questions
    """

    QUESTION_TEXT = "Which of the following visual cue(s) are from this meme and necessary to derive its intent?"
    ...
    def _get_init_questions(self) -> Tuple[List[Question] | Dict]:
        ...
        for img in self.img_set:
            matched_old_questions: List[Dict] = [
                ...
                if (
                    q['img'] == img 
                    and q['specific_type'] == 'visual_identification'
                )
            ]
            ...
            - The set correct answers in those questions are very likely to be identical
...
class VisualIdGenerator(AFGenerator):
    """
    Generator class for Visual Identification
    """
    ...
    def _prompt_template(self, image_caption, text, q: Question) -> str:
        ...
        if num_correct > 0:
            ...
            correct_derivs = "(There are no correct details recognized yet)"
        ...
        user_prompt = ' '.join([
            ...
            f"\nSomeone thinks the following details are visually visible on the meme and are important to understand its meaning:",
            f"\n{correct_derivs}",
            f"\nList {num_distractors} other statements that seem to be from the meme but are actually not.",
...
if __name__ == '__main__':
    args = af_init()
    generator = VisualIdGenerator()
    disciminator = AFDiscriminator()
    af = AFVisualId(generator, disciminator, SEMEVAL_IMAGES, args)
    af.run()